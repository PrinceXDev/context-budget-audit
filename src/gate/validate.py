...
def fail(msg):
    sys.stderr.write("gitlab-mr-gate: " + msg + "\n")
    raise SystemExit(1)
...
if not project:
    fail("project is required (e.g. gitlab-org/gitlab). Refusing to guess.")
if not mr_raw:
    fail("mr is required (the MR iid, e.g. 253806). Refusing to guess.")
...
# Host must look like a hostname, no scheme, no path, no credentials, no port tricks.
if not re.fullmatch(r"[A-Za-z0-9]([A-Za-z0-9._-]{0,251}[A-Za-z0-9])?", host):
    fail("gitlab_host must be a bare hostname (e.g. gitlab.com or gitlab.internal.corp), got: " + repr(host))
