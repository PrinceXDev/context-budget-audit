...
1. *"Can this one merge yet, and if not, what do I actually do?"* — you click
   through approvals, threads, the pipeline, the draft flag, CODEOWNERS.
2. *"What is stuck across the whole project?"* — nobody knows, because nobody
   opens sixty merge requests.
3. *"The pipeline is red — is that me?"* — the single most common blocker, and
...
The handoff is the point. The gate deliberately stops at *"the pipeline failed"*
...
the target branch's *current* pipeline would blame the author for a regression
...
- **Self-check before live data.** 47 cases against the gate's verdict logic and
  ...
  the logic must *refuse* to say. If one fails, the verdict is withheld.
...
"project not found", which made the plays unrunnable against *any* CE instance.
...
**Why live rows assert a verdict rather than *the* verdict.** For third-party
...
`registry play delete --version` is a *recoverable* delete, with
